...
def main(argv=None):
    ...
    if float(args.sigma_z) >= 0.5 * _shell_width:
        msg = (
            f"--sigma-z {args.sigma_z:g} is {args.sigma_z / _shell_width:.0%} of "
            ...
            f"--n-shells {max(int(args.z_depth / max(2.0 * args.sigma_z, 1e-9)), 1)} "
    ...
    sigma_z = lambda z: float(args.sigma_z) * np.ones_like(z)
    ...
    with h5py.File(out, "w") as f:
        ...
        g.attrs["sigma_z"] = float(args.sigma_z)
