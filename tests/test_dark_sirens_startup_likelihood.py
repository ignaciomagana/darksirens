...
def test_dark_sirens_likelihood_evaluates_once_before_sampling():
    ...
    # Reference value re-pinned after the review-fixes population corrections
    # (PR #359): the fiducial rate slope moved from gamma = 0 to the measured
    # kappa_z = 2.5 and the mass normalisers switched to the closed forms,
    # deliberately moving this value from 0.016343376536779530 by ~5e-2
    # relative. Previous re-pin: PR #191 (catalog-consistent selection
    # integral), 0.016464023225528157 -> 0.016343376536779530. Computed on
    ...
    np.testing.assert_allclose(float(value), 0.015520039551940812, rtol=1e-7)
