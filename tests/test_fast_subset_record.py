...
``tests/fast_subset.txt`` and ``docs/source/testing.md`` both stated "32 files,
...
def test_docs_record_agrees_with_the_manifest_record():
    if not DOCS.is_file():  # pragma: no cover - docs pruned from an sdist
        pytest.skip("docs/source/testing.md not in this checkout")
    ...
    assert n_doc is not None, (
        "docs/source/testing.md no longer states the gate size; it and "
