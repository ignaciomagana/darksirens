...
| `DARKSIRENS_XLA_CACHE is set but the XLA persistent compilation cache could not be enabled` | the cache root could not be created or written (a file in the way, a read-only or full filesystem) | the run compiles as if the cache were off; point `DARKSIRENS_XLA_CACHE` at a writable local directory |
