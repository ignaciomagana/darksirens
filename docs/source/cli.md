...
- `--kde_window W`: static window size for the per-sample catalog KDE — only the `W` galaxies index-nearest each sample's redshift (a window centred on the sample's insertion index in the z-sorted row) are evaluated. **Unset, the window is sized from the data at build time**: `darksirens.redshift.catalog.auto_kde_window` takes twice the largest number of galaxies any bound row holds within `X * max_row(sigma_eff)` on one side of a sample, at the widest `sigma_kde` the run can reach (its fixed value, or the prior's upper bound when it is sampled), so every sample's in-range galaxies provably fit and the evaluator never truncates. The former fixed default of `1024` truncated silently on denser rows (0.17 nats mean error on a DESI-like mixed spectro+photo row of 2158 galaxies); when the data-sized window exceeds it the factory says so, because a dense photo-z catalog then pays for the exact answer. A value pins `W`; `0` disables windowing entirely and evaluates the full row, the escape hatch for A/B validation. The chosen window is printed after the build.
- `--kde_window_nsigma X`: the sizing multiplier for that window — the window must hold every galaxy within `X * max_row(sigma_eff)` of a sample (default `8`; the kernel mass beyond it is `< exp(-32)` per galaxy). It enters only the build-time sizing; the evaluator itself performs one binary search per sample and no width arithmetic.
